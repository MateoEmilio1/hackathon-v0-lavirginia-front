...
function InspectionPanel({ onNewInspection }: { onNewInspection: (i: RecentInspection) => void }) {
  ...
  const hasResult = realVal?.status === "done";
      ...
      <div className="container-x">
        ...
        <div className="section-header">
          <div>
            ...
            <Reveal delay="2" as="p" className="section-sub">
              Subí una imagen de una bandeja de cápsulas para que el modelo Python la evalúe
              en los 3 ejes de calidad de la norma CC-2026.
            </Reveal>
          </div>
        </div>
        <div className="inspect-grid" style={{ gridTemplateColumns: "1fr" }}>
          <Reveal className="inspect-card panel">
            <div className="panel-head">
              <h3>{realVal?.status === "scanning" ? "Analizando…" : hasResult ? "Resultado de validación" : "Nueva validación"}</h3>
            ...
            <div
              ...
              onClick={() => !realVal || realVal.status !== "scanning" ? fileInputRef.current?.click() : null}
              ...
              style={{
                border: hasResult ? "2px solid var(--line)" : "2px dashed var(--bg-2)",
                borderRadius: 12,
                padding: "28px 24px",
                textAlign: "center",
                cursor: realVal?.status === "scanning" ? "not-allowed" : "pointer",
                opacity: realVal?.status === "scanning" ? 0.5 : 1,
                marginBottom: 24,
              ...
              {realVal?.status === "scanning" ? (
                <>
                  ...
                  <div style={{ fontSize: 12, color: "var(--ink-3)" }}>
                    Enviando al modelo Python · esperá la respuesta…
                  </div>
                </>
              ) : hasResult ? (
                <>
                  ...
                  <div style={{ fontSize: 12, color: "var(--ink-3)" }}>
                    Hacé clic o arrastrá un nuevo archivo JPG/PNG
                  </div>
                </>
              ) : (
                <>
              ...
              <div className="inspect-stack" key={realVal.item.id}>
                <div className="inspect-top">
                  ...
                  <div className="inspect-id-block">
                    <div className="row1">
                      <span className="pill">Archivo</span>
                      <span className="product">{realVal.item.product}</span>
                    </div>
                    <div className="cap-id">{realVal.item.id}</div>
                    <div className="variety">{realVal.item.variety}</div>
                  </div>
                  <div className={"verdict-badge " + (realVal.approved ? "ok" : "bad")}>
                    <div className="v-label">Resultado del modelo Python</div>
                    <div className="v-value">
                      <span className="vicon">{realVal.approved ? "✓" : "✕"}</span>
                      {realVal.approved ? "APROBADA" : "RECHAZADA"}
                    </div>
                    <div className="v-sub">
                      {realVal.approved
                        ? "Cumple CC-2026 · " + realVal.duration.toFixed(2) + " s"
                        : "Falla detectada · " + realVal.duration.toFixed(2) + " s"}
                    </div>
                  </div>
                </div>
                <div className="axes-grid">
                  {allAxes.map((axKey, i) => {
                    const ax = realVal.item.axes[axKey];
                    return (
                      <div key={axKey} className={"axis " + ax.status} data-i={i}>
                        <div className="axis-head">
                          <span className="axis-code">EJE {i + 1}</span>
                          <span className="axis-icon">{ax.status === "ok" ? "✓" : "✕"}</span>
                        </div>
                        ...
                        <div className="axis-reading">{ax.reading}</div>
                        <div className="axis-detail">{ax.detail}</div>
                      </div>
                    );
                  })}
                </div>
                <div className="inspect-foot">
                  <span style={{ fontSize: 12.5, color: "var(--ink-3)" }}>
                    Validado por modelo Python en {realVal.duration.toFixed(2)} s · {realVal.filename}
                  </span>
                </div>
              </div>
            ) : realVal?.status === "scanning" ? (
              ...
              <div className="inspect-stack" style={{ opacity: 0.4 }}>
                <div className="inspect-vis idle-vis" style={{ minHeight: 200 }}>
                  <div style={{ textAlign: "center", color: "var(--ink-2)", fontSize: 13, lineHeight: 1.6 }}>
                    ...
                    <div>de cápsulas para validarla con IA</div>
                  </div>
                </div>
              </div>
            )}
            {realVal?.status === "error" && (
              <div className="verdict-badge" style={{ marginTop: 12, background: "#fef2f2", borderColor: "#fecaca" }}>
                <div className="v-label" style={{ color: "#991b1b" }}>Error</div>
                <div className="v-value" style={{ color: "#b91c1c" }}>
                  <span className="vicon">!</span>ERROR DE CONEXIÓN
                </div>
                <div className="v-sub" style={{ color: "#b91c1c" }}>{realVal.message.slice(0, 80)}</div>
              </div>
            )}
          </Reveal>
        </div>
